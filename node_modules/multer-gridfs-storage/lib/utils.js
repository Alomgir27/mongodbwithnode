...
function compareUris(uri1, uri2) {
  ...
  for (let i = 0; i < hosts1.length; i++) {
    const hostObj = hosts1[i];
...
/**
 * Emits a deprecation warning tracking messages to avoid duplications.
 * Useful for deprecations that doesn't involve a complete function or method
 * @param {string} msg - The deprecation message to emit
 */
function deprecate(msg) {
  if (deprecations.indexOf(msg) === -1) {
    util.deprecate(() => {
